...
- **不构建、粘贴地址即装**：设置 → 插件 → 添加插件市场 → 粘贴 `https://github.com/Soonkeira/zcode-palette-presets` → 安装启用 palette-presets → 外观·主题包选用。共 **96 组明暗配色**（48 个主题包 = 6 主色 × 8 底色，浅色/深色各一组 token），与内置调色盘色值同源（生成脚本 `scripts/gen-palette-presets.ts`）。也可改选本地目录 `docs/palette-presets-marketplace/` 等效安装。
