...
- **预设市场（粘贴地址即装，仅限本 fork 构建）**：官方原版 ZCode 没有主题插件功能（相关代码在本 fork 未上游的提交中），装了市场也不会出现「主题包」入口。在本 fork 构建的客户端里：设置 → 插件 → 添加插件市场 → 粘贴 `https://github.com/Soonkeira/zcode-palette-presets` → 安装启用 palette-presets → 外观·主题包选用。共 **96 组明暗配色**（48 个主题包 = 6 主色 × 8 底色，浅色/深色各一组 token），与内置调色盘色值同源（生成脚本 `scripts/gen-palette-presets.ts`）。也可改选本地目录 `docs/palette-presets-marketplace/` 等效安装。
