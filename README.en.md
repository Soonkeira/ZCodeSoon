...
- **Preset marketplace (paste the URL; requires this fork's build)**: official upstream ZCode has no theme-plugin support (the feature lives in this fork's un-upstreamed commits), so installing the marketplace there will not surface any "Theme package" entry. In a build of this fork: Settings → Plugins → Add marketplace → paste `https://github.com/Soonkeira/zcode-palette-presets` → install & enable palette-presets → choose under Appearance → Theme package. **96 light/dark combinations** across 48 theme packages (6 primaries × 8 surfaces, one light and one dark token set each), generated from the same source as the built-in palette (`scripts/gen-palette-presets.ts`). The local directory `docs/palette-presets-marketplace/` works as an equivalent alternative.
